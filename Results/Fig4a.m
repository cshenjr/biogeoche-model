...
%   tt = [CB21,mCB21,CB22;mCB22,CB32,mCB32,CB858,mCB858,CB41,mCB41,CB43,mCB43,...
%       CB51,mCB51,CB52,mCB52,CB53,mCB53,CB62,mCB62,CB64,mCB64];
%   tt = [CB21,CB22;CB32,CB858;CB41,CB43;CB51,CB52;CB53,CB62];
%   gp = [10*ones(size(CB21));12*ones(size(CB21));20*ones(size(CB22));22*ones(size(CB22));...
%       30*ones(size(CB32));32*ones(size(CB32));...
%       40*ones(size(CB858));42*ones(size(CB858));50*ones(size(CB41));52*ones(size(CB41));...
%       60*ones(size(CB43));62*ones(size(CB43));70*ones(size(CB51));72*ones(size(CB51));...
%       80*ones(size(CB52));82*ones(size(CB52));90*ones(size(CB53));92*ones(size(CB53));...
%       100*ones(size(CB62));102*ones(size(CB62));110*ones(size(CB64));112*ones(size(CB64))];
  ...
  ylabel('DIC (µ mol/kg)','fontweight','bold','fontsize',14)
