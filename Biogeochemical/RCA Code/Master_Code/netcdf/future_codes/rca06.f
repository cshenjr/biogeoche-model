...
!        ELSEIF(ITVNCOPT.EQ.1) THEN
!          READ(37,1000) COMMENT
!          READ(37,'(A10,6X,A4)',ERR=950)   PNAME,TWARPTVF
!          ISCALTVF=IUNITCHECK(TWARPTVF,'TWARPTVF')
!          TVpmTflag(:,I)=(/1,2/)
!          CALL ncprm_1Dinterp(ncID_prm,
!     .                        idTVpm_time(I),idTVpm(I),TVpmTflag(:,I),
!     .                        NXFUNT(I),MFUNC(I),BFUNC(I)) 
!          WRITE(OUT,2001)   TRIM(ADJUSTL(PNAME)), NXFUNT(I),BFUNC(I),
!     .                      TRIM(ADJUSTL(Vinfo(3,idTVpm(I))))
! 2001     FORMAT(' LOAD Next ',A10,' at DAY ',F10.2,' = ',E9.2,A)
!        ENDIF
  300 CONTINUE
!
!--------------------------------------------------------------
