...
C        INITIALIZATION
      ...
      NXPRTD=IPRNTDSECS
      NXPRTG=IPRNTGSECS
      NXPRTR=IPRNTRSECS   !! Yun Li, UMCES/HPL Jun-04-2011
      IWRTADDM=0
      IADDMDEBUG=0
      INITMB=1
      ...
      IF(TZERO.NE.0.)   THEN
         ...
         NXPRTR = ITIMESECS+IPRNTRSECS  !! Yun Li, UMCES/HPL, Jun-04-2011
         IF(MASSBAL.EQ.1) THEN
...
c$doacross local(iy,ix,iz,is,ie)
     ...
     .              + AVECT(IX,IY,IZ,6)*CARAY(IX,IY,IZ+1,ISYS)
	 	      TESTQQ = DM(IX,IY,IZ)*DT/BVOLN(IX,IY,IZ)
  123     CONTINUE
  125   CONTINUE
C      BOTTOM LAYER
c$doacross local(iy,ix,is,ie)
      ...
      DO 135 IZ=1,NZ
       DO 135 IY=1,NY
        ...
        DO 134 IX=IS,IE
		 ...
		 TESTQQ = CDARAY(IX,IY,IZ,ISYS)*DT/BVOLN(IX,IY,IZ)
