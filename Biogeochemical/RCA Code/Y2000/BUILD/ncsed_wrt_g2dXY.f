      SUBROUTINE ncsed_wrt_g2dXY(ncID,idvar,Vvalue,mX,mY)
!
!=======================================================================
!  WRITE sediment variables into NetCDF output                         !
...
!-----------------------------------------------------------------------
!
      SUBNA=TRIM(ADJUSTL(RCANA))//' when CALL ncsed_wrt_g2dXY.f'
!
!----------------------------------------------------------------------
...
!
      IF(Iinfo(idvar).EQ.g2dvar) THEN 
        ...
        mcount=(/mX,mY,1/)
        mstart=(/ 1, 1,ncIREC_sed/)
      ELSE
        WRITE(OUT,*) 'ERROR: misuse of ncsed_wrt_g2dXY.f ',
...
! set land values to default spval (for ncview display)
      IF(FSM(mX,mY).EQ.0.0) THEN
         Vvalue_mask(1)=spval
      ELSE
         Vvalue_mask(1)=Vvalue
      ENDIF
...
! put values
     ...
     .                    count=mcount)
      ...
      END SUBROUTINE ncsed_wrt_g2dXY
